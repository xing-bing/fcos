...
class Fcos_Loss(nn.Module):
    def __init__(self, model):
        ...
        det = model.module if is_parallel(model) else model
        for k in 'strides', 'limit_range', 'sample_radiu_ratio':
            setattr(self, k, det.yaml[k])
    ...
    def get_target(self, cls_logits, gt_boxes, classes):
        ...
        for level in range(len(cls_logits)):
            ...
            # ----------------------------------------------------------------#
            cnt_targets = (
                    (left_right_min * top_bottom_min) / (left_right_max * top_bottom_max + 1e-10)).sqrt().unsqueeze(
                dim=-1)
    ...
    def compute_cnt_loss(self, preds, targets, mask):
        ...
        for batch_index in range(batch_size):
            ...
            batch_loss = nn.functional.binary_cross_entropy_with_logits(input=pred_pos, target=target_pos,
